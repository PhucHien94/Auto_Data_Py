...
```
Auto_Data_Py/
├── scripts/                 generator, runner, and reporting scripts
│   └── loadtest/             k6 load-test script + query fixture
├── data/
...
```powershell
python scripts\generate_master_testset.py --store nsg --out-dir output\golden_testsets\nsg
...
```powershell
python scripts\run_autoscript.py `
...
python scripts\run_autoscript.py --dataset-dir output\golden_testsets\nsg --env staging
```
...
```powershell
python scripts\export_manual_qc_assets.py --dataset-dir output\golden_testsets\nsg --out-dir output\manual_qc\nsg
```

Writes a Postman collection + environment + data CSV, and `scripts/loadtest/k6_search_test.js` + `k6_queries.json`.

## Legacy / ad-hoc test data generation

```powershell
python scripts\generate_text_testdata.py --ndjson data\ProductInfo\mart_en_nsg_product.ndjson --out-dir output\text_testdata
python scripts\generate_multilang_queries.py --v1 data\ProductInfo --out output\text_testdata\multilang_queries.xlsx --max-per-file 200
python scripts\sample_queries.py --in output\text_testdata\multilang_queries.xlsx --out output\text_testdata\sample_queries.xlsx --size 1000
python scripts\runner_text_search.py --excel output\text_testdata\text_testdata.xlsx --inputs output\text_testdata\sample_inputs.ndjson --api-url "https://api.example.com/search" --method GET --param q --topn 5 --header "Authorization: Bearer <TOKEN>"
```

## Files of interest

- `scripts/generate_master_testset.py` — golden dataset generator (Part 1 of the strategy)
- `scripts/run_autoscript.py` + `lib_scoring.py` + `lib_search_client.py` — run + score + report against a live API (Part 2/3)
- `scripts/export_manual_qc_assets.py` — Postman/k6 asset export (Part 4)
- `scripts/generate_text_testdata.py`, `generate_multilang_queries.py`, `generate_image_testdata.py`, `sample_queries.py`, `runner_text_search.py`, `add_results_template.py`, `preview_testcase.py`, `translate_excel.py` — earlier ad-hoc generator/runner scripts, still usable standalone
